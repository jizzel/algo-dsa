...
//    }
     ...
     * Merges two binary trees by adding the values of nodes that overlap.</h4>
     * </h4>
     * If two nodes overlap, their values are added together, and the result is assigned to the merged tree's node.</h4>
     * If one of the trees has a `null` node at a specific position, the non-null node from the other tree is used.</h4>
     * </h4>
     ...
     * </h4>
     * Time Complexity: O(n) - where n is the minimum number of nodes in the two trees.</h4>
     * Space Complexity: O(n) - due to the recursive call stack.</h4>
     */
